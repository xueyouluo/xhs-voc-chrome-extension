// 加入prompt变量
export function sellingPointPrompt({query, context}) {
    return `请根据下面文章的内容，总结${query}的特点，使用小红书的风格，字数不超过100
...
其中：
- 包含了正面和负面的情感分析结果。
- 正负面分别包含了主题、提及次数、提及占比和原声。

注意：
- 使用小红书风格，有标题和正文。
- 标题和小标题要吸引人，并且以markdown格式输出。
- 内容生动活泼，有吸引力，内容详实，有理有据， 给出提及次数和占比。
- 每个主题需要先整体介绍情况，然后总结一下用户的原声以增强说服力，避免直接引用原声进行堆砌。
- 用第一人称来叙事，如"大家觉得"，"很多人说"这样的语句，不要出现“用户觉得”这样的第三人称。
...
export function singleNoteABSAPrompt({query, context}) {
...
3. 只输出一个表格，不要输出其他内容。
4. 不输出与查询"${query}"无关的内容，如查询为"北京"，那么不输出opinion target为"上海"的内容。
